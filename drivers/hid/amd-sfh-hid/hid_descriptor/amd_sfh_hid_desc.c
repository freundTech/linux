...
#define KEYBOARD_UNAVAILABLE_MASK				BIT(1)
#define POSE_MASK						GENMASK(31, 28)
#define HINGE_ANGLE_MASK					GENMASK(15, 4)
			   ...
			   struct amd_input_data *in_data)
{
	...
	case tms_idx: /* tablet mode switch */
		...
		tms_value = readl(privdata->mmio + AMD_C2P_MSG(3));
		tms_input.hinge_angle = (tms_value & HINGE_ANGLE_MASK) >> 4;
		tms_input.keyboard_unavailable = !!(tms_value & KEYBOARD_UNAVAILABLE_MASK);
		tms_input.pose = (tms_value & POSE_MASK) >> (7 * 4) ;
